...
let defaultSetting = {
    ...
    'block_selectors': ['*[style*="opacity"]', '*[style*="background"]', '*[style*="z-index"]', '*[style*="display"]', '*[style*="margin: 0"]', '*[style*="padding: 0"]'],
...
if ($response.statusCode === 200 && (url.includes('html') || (type && type.includes("text")))) {
    ...
    if (htmLen < settingCfg.preBlock) {
        console.log(`pre block invalid html , size: ${htmLen} , ContentLength: ${$response.headers['Content-Length']}`);
        $done({});
        return;
    }
    ...
    console.log(`ignored,type=${type},code=${$response.statusCode}`);
    $done({});
    return;
}
function configProxy(url) {
    ...
    let match_cfg = matchSpecialConfig(url);
    if (!match_cfg) return;
    let changed = false;
    for (let k in match_cfg) {
        settingCfg[k] = match_cfg[k];
        changed = true;
    }
...
function calcPrvANex(html) {
    const calc_prvs = settingCfg.prev_texts;
    const calc_nexts = settingCfg.next_texts;
    const calc_mls = settingCfg.dir_texts;
    const targetDom = settingCfg.pnd_doms;
    const calcRgx = (str) => new RegExp(`<(${targetDom.join('|')})\\s+[^>]*?(href|class|id)\\s*=\\s*(['"])([^'"]*?)\\3[^>]*?>[^<]*?${str}[^<]*?</\\1>`, 'g');
    const itemRule = /[^=]*?=(["'])[^'"]*?\1/;
