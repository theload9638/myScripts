...
let defaultSetting = {
    ...
    'block_selectors': ['*[style*="background"]', '*[style*="display"]','*[style*="opacity"]','*[style*="margin: 0px"]', '*[style*="z-index"]','*[style*="padding: 0px"]'],
...
if ($response.statusCode === 200 && (url.includes('html') || (type && type.includes("text")))) {
    ...
    try {
        ...
        if (/utf-?8/i.test(charset) || charset.toLowerCase().startsWith('utf')) {
            ...
            bodyStr+=`<script>for(let i of document.forms){i.acceptCharset='${charset}';}</script>`;
        }
