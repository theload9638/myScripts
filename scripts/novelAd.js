    ...
    try {
        ...
        if (/utf-?8/i.test(charset)) {
            ...
            if (/^https?:\/\/hlib\.cc/.test(url)) {
                ...
                styleStr += '.slide,img{display:none !important;pointer-events: none !important;}';
...
function applyFloatyW(html) {
    ...
    scp += `const KillRender={initialize:false,running:false,rafId:null,count:0,times:${settingCfg.block_times},observer:null,TAGS:new Set(['IMG','VIDEO','IFRAME','OBJECT','EMBED','AUDIO','PICTURE','SOURCE','SVG','IMAGE','FRAME','INS','*[style*="opacity"]','*[style*="background"]','*[style*="z-index"]','*[style*="display"]']),removeIfTarget(node){if(node.nodeType!==1)return;if(this.TAGS.has(node.tagName)){node.remove();return}if(node.querySelectorAll){node.querySelectorAll([...this.TAGS].join(',')).forEach(el=>el.remove())}},cleanStyleSheets(){for(const sheet of document.styleSheets){let rules;try{rules=sheet.cssRules}catch(e){continue}if(!rules)continue;for(let i=rules.length-1;i>=0;i--){const rule=rules[i];if(rule.style&&rule.style.backgroundImage&&rule.style.backgroundImage.includes('url')){rule.style.backgroundImage='none'}}}},sweep(){document.querySelectorAll([...this.TAGS].join(',')).forEach(el=>el.remove());this.cleanStyleSheets()},loop(){if(!this.running)return;this.sweep();this.count++;if(this.count>=this.times){this.stop();return}this.rafId=requestAnimationFrame(()=>this.loop())},initObserver(){this.observer=new MutationObserver(mutations=>{for(const m of mutations){for(const node of m.addedNodes){this.removeIfTarget(node)}}});this.observer.observe(document.documentElement,{childList:true,subtree:true})},hookDOMInsert(){const methods=['appendChild','insertBefore','replaceChild'];methods.forEach(method=>{const original=Node.prototype[method];Node.prototype[method]=function(...args){const node=args[0];if(node&&node.tagName&&KillRender.TAGS.has(node.tagName)){return node}return original.apply(this,args)}})},hookCanvas(){const originalGetContext=HTMLCanvasElement.prototype.getContext;HTMLCanvasElement.prototype.getContext=new Proxy(originalGetContext,{apply(target,thisArg,args){const type=args[0];if(type==='2d'||type==='webgl'||type==='webgl2'||type==='experimental-webgl'){return null}return Reflect.apply(target,thisArg,args)}})},hookDark(){window.open=function(){return false};const originalHrefDescriptor=Object.getOwnPropertyDescriptor(window.location,'href');const currentDomain=window.location.hostname;const allowedDomains=[currentDomain];Object.defineProperty(window.location,'href',{set:function(newHref){try{const targetUrl=new URL(newHref,window.location.href);const targetDomain=targetUrl.hostname;const isAllowed=allowedDomains.some(domain=>{return targetDomain===domain||targetDomain.endsWith('.'+domain)});if(isAllowed){originalHrefDescriptor.set.call(this,newHref)}else{return false}}catch(e){originalHrefDescriptor.set.call(this,newHref)}},get:function(){return originalHrefDescriptor.get.call(this)},configurable:true,enumerable:true})},start(){if(this.running){return}if(!this.initialize){this.hookDark();this.hookDOMInsert();this.hookCanvas();this.initialize=true}this.running=true;this.initObserver();this.count=0;this.loop()},stop(){this.running=false;if(this.rafId){cancelAnimationFrame(this.rafId);this.rafId=null}if(this.observer){this.observer.disconnect();this.observer=null}}};`;
