...
if ($response.statusCode === 200 && (url.includes('html') || (type && type.includes("text")))) {
    ...
    try {
        ...
        if (/utf-?8/i.test(charset)) {
            ...
            }else if(/^https?:\/\/www\.69shuba\.com\/(book|txt)/.test(url)){
                html = html.replace(/<script\s*>[^>]*?<\/script>/gs, '');
            }
        }
