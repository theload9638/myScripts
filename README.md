...
'jsonjq-response-body'
    ...
    * - .代表顶层对象 或 属性连接符
    * - ,代表输出多个表达式的结果用来拼接各表达式
    * - |代表管道操作符,用于将前结果传递给后面的输入,可多次使用,相当于执行多次jq
    * - length (不能包含其他语法),代表对象属性个数/数组长度/字符串长度
    * - del(属性表达式) 用于删除属性
    * - select(布尔表达式) 用于筛选 &gt; &lt;
    * - [表达式] 用于将{}转[{}]
    * - keys (不能包含其他语法)用于获取对象的所有属性名称数组
    * - 属性=值
...
```js
//中间人攻击配置
skip_src_ip = 1.1.1.1,2.2.2.2 //该tcp连接的源IP将不被处理,支持 * 和 ?
skip_src_ip = 1.1.1.1,2.2.2.2 //该tcp连接的目标IP将不被处理,支持 * 和 ?
hostname = *.a.com , c?.b.com 
   1.//中间人攻击的主机配置,该配置影响抓包功能,支持 * 和 ?
   2.//前缀- 用于排除某主机名,例如 -*.anc.com
   3.//单独的 * 将匹配所有主机名
```
